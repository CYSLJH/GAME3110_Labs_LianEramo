...
public class NetworkedClient : MonoBehaviour
{
    ...
    LinkedList<string> receivedPartyData;
    ...
    private void ProcessReceivedMsg(string msg, int id)
    {
        ...
        if(signifier == ServerToClientSignifiers.PartyDataTransferStart)
        {
            receivedPartyData = new LinkedList<string>();
        }
        else if(signifier == ServerToClientSignifiers.PartyDataTransfer)
        {
            receivedPartyData.AddLast(msg);
        }
        else if (signifier == ServerToClientSignifiers.PartyDataTransferEnd)
        {
            AssignmentPart2.LoadPartyFromReceivedData(receivedPartyData);
        }
